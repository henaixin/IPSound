...
def argsProcess():
    ...
    parser.add_argument('--appId', type=str, default='368d8439c0d04e58a80fd118ee415108', help='')
    parser.add_argument('--appCode', type=str, default='509f43490954400fb1a6a73655cb85ae', help='')    
    ...
    parser.add_argument('--ip',type=str, default=r"127.0.0.1",help='')
...
class IPSound:
    # 初始化参数
    def __init__(self, appId, appCode,ip):
        self.ip = ip
        self.appId = appId
        ...
        self.uploadAudios = {}
        self.urls = {
            "token":f"https://{self.ip}:11000/sos/v1/mntn/account/appId/token",
            "tokenRefresh":f"https://{self.ip}:11000/sos/v1/mntn/account/refresh/token",
            "getSoundsInfo":f"https://{self.ip}:11000/sos/v1/mntn/business/audioFile/list",
            "uploadSound":f"https://{self.ip}:11000/sos/v1/mntn/business/audioFile/upload",
            "createTaskAndStartPlay":f"https://{self.ip}:11000/sos/v1/mntn/business/play/task/start",
            "stopTask":f"https://{self.ip}:11000/sos/v1/mntn/business/play/task/stop",
            "delTask":f"https://{self.ip}:11000/sos/v1/mntn/business/play/task/del",
            "taskState":f"https://{self.ip}:11000/sos/v1/mntn/business/play/task/detail",
            "delAudio":f"https://{self.ip}:11000/sos/v1/mntn/business/audioFile/del",
...
def callback():
    ...
    if dev2task and deviceId in dev2task.keys():
        ...
        if taskCode and status and status == "finished":
            ...
            #此次播放跳过
            logger.warning("taskCode:", taskCode, "status:", status)
            logger.warning(f"skip this alarm deviceId{deviceId},audioName{audioName},algorithmId{jsonData['algorithmId']}")
...
if __name__ == '__main__':
    args = argsProcess()
    logger.info(f"http://{args.ip}:{args.port}/callback")
    IPS = IPSound(args.appId, args.appCode,args.ip)
